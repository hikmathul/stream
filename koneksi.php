...
// tambah user PERLU PERBAIAN
function tambahUser($data) {
	global $koneksi;
	$username = htmlspecialchars($data['username']);
	$password = password_hash($data['password'], PASSWORD_BCRYPT);
	$nama_lengkap = htmlspecialchars(addslashes($data['nama_lengkap']));
	$photo_profile = htmlspecialchars("assets/img/img_profiles/".($data['photo_profile']));
	mysqli_query($koneksi, "INSERT INTO tb_user VALUES ('', '$username', '$password', '$nama_lengkap', '$photo_profile')");
	riwayat($_SESSION['id_user'], "Berhasil menambah user");
...
function hapususer($id_user) {
	...
	if ($photo_profile != 'default.png') {
		unlink('../assets/img/img_profiles/' . $photo_profile);
	}
	mysqli_query($koneksi, "DELETE FROM tb_user WHERE id_user = '$id_user'");
	riwayat($id_user, "Berhasil menghapus user");
	return mysqli_affected_rows($koneksi);
}
...
function tambahFilm($data) {
	global $koneksi;
	$nama_film = htmlspecialchars(addslashes(ucwords($data['nama_film'])));
	$link_film = htmlspecialchars(addslashes($data['link_film']));
	$link_download = htmlspecialchars(addslashes($data['link_download']));
	$cover_film = htmlspecialchars(addslashes($data['cover_film']));
	...
	mysqli_query($koneksi, "INSERT INTO tb_film VALUES ('', '$nama_film','$link_film','$link_download', '$cover_film', '$tanggal_diposting', '$id_genre', '$id_user')");
...
function hapusFilm($id) {
	...
	riwayat($_SESSION['id_user'], "Berhasil menghapus film $nama_film");
	return mysqli_affected_rows($koneksi);
}
...
function ubahFilm($data) {
	...
	$link_film = htmlspecialchars($data['link_film']);
	$link_download = htmlspecialchars(addslashes($data['link_download']));
	$cover_film = htmlspecialchars(addslashes($data['cover_film']));
	$tanggal_diposting = time();
	$id_genre = htmlspecialchars($data['id_genre']);
	...
	// }
	mysqli_query($koneksi, "UPDATE tb_film SET nama_film = '$nama_film', link_film = '$link_film', link_download = '$link_download', cover_film = '$cover_film', tanggal_diposting = '$tanggal_diposting', id_genre = '$id_genre', id_user = '$id_user' WHERE id_film = '$id_film'");
...
function laporan($data) {
	global $koneksi;
	// jika id film sudah ada maka tidak akan diinputkan lagi
	$id_film = htmlspecialchars($data['id_film']);
	$cek = mysqli_query($koneksi, "SELECT * FROM tb_laporan WHERE id_film = '$id_film'");
	if (mysqli_num_rows($cek) > 0) {
		return false;
	} else {
		$id_film = ucwords($data['id_film']);
		$nama_film = ucwords($data['nama_film']);
		$tanggal_laporan = time();
		mysqli_query($koneksi, "INSERT INTO tb_laporan VALUES ('', '$id_film', '$nama_film', '$tanggal_laporan')");
	}
